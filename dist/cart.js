...
(() => {
 ...
 const render=()=>{
  ...
  if(!total){body.innerHTML='<div class="sc-empty"><p>A space for your<br /><em>next favourite.</em></p><span>'+(!healthy?(loaded&&!refreshing?'Stock could not be checked. Please try again.':'Checking availability…'):'Your bag is empty.')+'</span><a href="#top" data-bag-explore>Explore the hoodie <span aria-hidden="true">↗</span></a></div>';summary.innerHTML='';}
   ...
   // Preserve the action node and its hover/focus state while stock is refreshed.
   if(!summary.firstElementChild)summary.innerHTML=`<div class="sc-subtotal"><span>Subtotal</span><strong>${money(total*product.price)}</strong></div><p>Held for 30 minutes from your first addition. Changes and order requests do not extend the timer.</p><button type="button" class="sc-request" data-checkout-open>Arrange your order <span aria-hidden="true">↗</span></button><a class="sc-email-alternative" href="mailto:${product.email}?subject=${encodeURIComponent('Spectre Hoodie — bag enquiry')}&body=${encodeURIComponent(message)}">Or enquire by email ↗</a><p>No payment is taken. Delivery is additional. Supplied without drawstrings; ask to add them if you prefer.</p>`;
   summary.querySelector(".sc-subtotal strong").textContent=money(total*product.price);
   summary.querySelector("[data-checkout-open]").disabled=busy||!healthy;
   summary.querySelector(".sc-email-alternative").href=`mailto:${product.email}?subject=${encodeURIComponent("Spectre Hoodie — bag enquiry")}&body=${encodeURIComponent(message)}`;
  }
  tick();notifyStock();
  if(restore&&dialog.open&&(!focused.isConnected||focused.matches(':disabled'))){const next=restore.action?body.querySelector(`[data-bag-action="${restore.action}"][data-size="${restore.size}"]:not(:disabled)`):restore.explore?body.querySelector('[data-bag-explore]'):restore.checkout?summary.querySelector('[data-checkout-open]:not(:disabled)'):summary.querySelector('a');(next||dialog.querySelector('[data-bag-close]')).focus({preventScroll:true});}
 };
