...
(() => {
  ...
  panel.innerHTML = `
    ...
    <div class="sc-checkout-result" data-checkout-result hidden><p class="sc-checkout-kicker" data-checkout-reference></p><h3 tabindex="-1" data-checkout-result-title></h3><p data-checkout-result-message></p><p class="sc-checkout-hint">This is a request, not a completed order. No payment has been taken, and the original reservation time still applies.</p><a class="sc-request" data-checkout-result-email href="mailto:hello@spectre-studio.co">Follow up by email <span aria-hidden="true">↗</span></a><button class="sc-checkout-back" type="button" data-checkout-result-back>Back to your selection <span aria-hidden="true">→</span></button></div>`;
  ...
  form.elements.contactMethod.addEventListener('change', () => {
    ...
    $('[data-checkout-contact-hint]').textContent = method === 'email' ? 'We’ll use this to discuss your request.' : 'Include your @username so we can find the right account.';
    paintSubmit();
  });
